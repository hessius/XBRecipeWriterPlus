...
- **`library/`** — plain TypeScript domain classes, no React. This is where all card and business logic lives.
  ...
  - `librarySort.ts`, `libraryFilters.ts`, `libraryQuery.ts` — the library list is a SQL query now. Sort axes and direction words live in `librarySort.ts`, stock filter vocabulary and suppression live in `libraryFilters.ts` — three id shapes, a bare stock id, `tag:` for a shelf somebody built and `sharedBy:` for a person who shared, and `availableFilters` is the single suppression gate (3-member floor, 80% ceiling) both the chips and the grid call. `mine` (`sharedByKey IS NULL`) is the complement of every author shelf, so it is normally over the ceiling and hidden until enough of the library came from other people. `quickBrew`/`slowBrew` read the `brewSeconds` index column, which is `plannedSeconds(pours)` and NULL for a stageless recipe, so the shelves agree with the staircase the brew screen draws. The ratio pair is `shortRatio`/`longRatio`, renamed from `strong`/`long`/`mild` because a shelf sorting on `ratio` alone cannot promise strength; `fewStages` (`pourCount <= 2`) deliberately contains `singlePour`, the only overlap between two stock shelves — and `libraryQuery.ts` builds the one statement that searches, filters and orders recipe UUIDs/blobs. Do not rebuild this in a screen or hook.
  ...
  - `BrewDatabase.ts` — three tables, two lifetimes. `brews` is one short row per brew and is kept until the user deletes it; `brew_samples` is roughly 2,400 rows per brew and is swept by the retention setting. `hasStream` tells a record whether its stream survived the sweep — a record without a stream still shows its figures, it just has no trace to draw. The values are copied at write time: recipe name, accent, and volumes are snapshotted so renaming or deleting a recipe cannot rewrite history. `rating`, `note` and `pinned` carry the user's verdict; `judge()` writes it and pins the row in the same statement, and `sweep()` filters pinned rows out **before** it slices, so holding one back never costs an ordinary brew its trace. `watched` is explicit, not inferred: rating a recipe the machine never brewed writes an `unobservedBrew()` row with `watched` false, and a brew refused for `notEnoughWater` also has zero water and zero held time but *was* watched. `hydrate` emits the flag only when false, so a watched record is byte-for-byte what it was before the column existed.
...
- **`hooks/`** — the stateful logic the screens used to inline. `useRecipeEditor` owns the recipe and every mutation on it; `useCardWriter` owns the NFC write path; `useRecipeLibrary` owns the library and restore; `useLibraryQuery` owns the rail's transient search/filter state plus persisted sort settings and produces the `LibraryQuery`; `useBackup` owns export/import of backup files; `useSetting` binds one setting to state; `useRecipeImport` owns the import lookup state machine; `useCollapsibleHeader` the home screen's header; `useBrewHistory` the history list with optional stream loading; `useTraceAnimation` the animation clock for the brew chart's pre-pour phases. Put new screen logic here rather than growing a route file back to 800 lines.
  - Evidence and shelf art are read on the same revision counter as the list: `library.evidence[uuid]` is what a recipe's brews add up to (a recipe with none has no entry at all, and the card then draws nothing), and `library.shelfMarks[shelfId]` is the first three members' accents and profiles. Both are separate grouped reads, not columns on the library query, because what a card says must not depend on what the rail asked.
  - `useRecipeLibrary().recipes` means "the answer to the current query", not "the whole table". When a caller needs whole-library answers, use `library.allRecipes()` for hydrated recipes, `librarySize` for the count, and `filterCounts` for stock shelf counts. Do not use `library.recipes` for backup, dedupe, headers, or count copy unless the visible query result is exactly what you mean.
- **`components/`** — Tamagui presentational/dialog components. Declare them at module scope: a component defined inside another component's body is a new type on every render, so React remounts it and throws away its state. That bug has already been fixed twice here.
  - `ShelfMark` is the 44 pt square on a shelf tile, and the size is the contract: three art candidates (`mosaic`, `profiles`, `glyph`) plus the `hybrid` default all draw at exactly that size so a variant swap cannot reflow the grid. The hybrid gives an auto shelf its glyph from `constants/shelfGlyphs.ts` and a manual shelf its members' pour profiles, so the art says which kind of shelf it is. Which one draws is the `shelfMarkVariant` LABS setting, which goes when the tester group answers. At most three members are read, enforced in the component as well as in the query.
...
- **The editor has three decks, and identity lives on ABOUT**, not on BREW where it used to. `components/AboutDeck.tsx` holds four sections: the note (`NoteSection`), the pod and the Recipe ID with it (`PodSection`), where a recipe arrived from (`FromSection`) and one line of brew history (`HistorySection`). BREW holds brew parameters alone. The name has no field on any deck: it is the screen header, renamed through `RenameSheet.tsx` from all three. An empty `recipe.name` is not a missing value, it means follow the pod, which is why USE THIS NAME clears the name rather than copying `xbloomName` into it.
...
A brew can be judged: five whole stars and a free-text note, asked for on the finished brew and on the record, through `components/BrewJudgement.tsx` and `hooks/useBrewHistory.ts`'s `useBrewJudgement`. **0 is unrated, not a verdict of nothing** — `BrewStars` draws nothing where a brew has no stars, and the library's Rating axis averages `NULLIF(rating, 0)` so an unrated brew cannot drag an average down. Judging pins implicitly, because a rating whose trace the next sweep took is a rating the user cannot act on. The control renders **outside** the `ViewShot`, so a shared card carries the brew and not the question.

A backup carries brew **records** but never their sample streams: the streams are thousands of rows a brew and expire under a retention the user chose. Restored records arrive with `hasStream: false` and are merged by `id`, **never overwriting** — the row already on the device may carry a verdict given after the backup was made. `BACKUP_VERSION` did not move; an older file simply has no `brews` key.
