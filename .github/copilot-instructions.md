...
- **`library/`** — plain TypeScript domain classes, no React. This is where all card and business logic lives.
  ...
  - `librarySort.ts`, `libraryFilters.ts`, `libraryQuery.ts` — the library list is a SQL query now. Sort axes and direction words live in `librarySort.ts`, stock filter vocabulary and suppression live in `libraryFilters.ts` — three id shapes, a bare stock id, `tag:` for a shelf somebody built and `sharedBy:` for a person who shared, and `availableFilters` is the single suppression gate (3-member floor, 80% ceiling) both the chips and the grid call. `mine` (`sharedByKey IS NULL`) is the complement of every author shelf, so it is normally over the ceiling and hidden until enough of the library came from other people. `quickBrew`/`slowBrew` read the `brewSeconds` index column, which is `plannedSeconds(pours)` and NULL for a stageless recipe, so the shelves agree with the staircase the brew screen draws. `mild` was `long` until the duration pair needed the word — and `libraryQuery.ts` builds the one statement that searches, filters and orders recipe UUIDs/blobs. Do not rebuild this in a screen or hook.
