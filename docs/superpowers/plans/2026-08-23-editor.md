...
export type HelpTopic = keyof typeof RECIPE_HELP;
...
export default function Stepper({label, value, min, max, step, accent, unit, onChange}: Props) {
    ...
    // The value can change from outside — auto fix rewrites every stage volume
    // at once — and a stale draft would keep showing the old number.
    useEffect(() => {
        setDraft(null);
    }, [value]);
    ...
    return (
        <XStack alignItems="center" gap="$2"
            ...
            {/* Two modes rather than a permanently editable field. Doto is a
                ...
                is Doto and the brief editing state is the body font. */}
            {editing ? (
...
export default function FieldRow({topic, helpStyle, explaining, onHelp, children}: Props) {
    ...
    const showMarker = hasDetail && helpStyle === "markers" && onHelp !== undefined;
...
function StageValue(props: {
    ...
    return (
        <XStack flex={1} alignItems="center" justifyContent="space-between"
            ...
            <Stepper label={props.label} value={props.value} min={props.min}
                     max={props.max} step={props.step} accent={props.accent}
                    ...
                    return (
                        <Pressable key={option.value} accessibilityRole="radio"
                            ...
                            <Text fontSize={10.5} fontWeight="600"
                                  ...
                                  borderRadius="$2"
                                  backgroundColor={selected ? props.accent : "transparent"}
                                  color={selected ? palette.base : palette.dim}>
                                {option.label}
                            </Text>
                        </Pressable>
...
describe("HelpSheet", () => {
    it("shows one topic when asked for one", async () => {
        ...
        expect(screen.getByText(RECIPE_HELP.grindSize.label)).toBeTruthy();
        expect(screen.getByText(RECIPE_HELP.grindSize.detail!)).toBeTruthy();
        expect(screen.queryByText(RECIPE_HELP.ratio.label)).toBeNull();
    });
    ...
    it("shows every topic that has something to say when asked for all", async () => {
        ...
        expect(screen.getByText(RECIPE_HELP.grindSize.label)).toBeTruthy();
        expect(screen.getByText(RECIPE_HELP.ratio.label)).toBeTruthy();
    });
