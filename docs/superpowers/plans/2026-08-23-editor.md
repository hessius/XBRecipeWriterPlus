...
the recipe's own profile as a watermark. **Nothing in it is tappable**, and the
test that says so counts touch responders rather than buttons, because a bare
`Pressable` declares no role and would slip past a query for one. The name
...
It carries its own tests in `components/__tests__/XbrwSheet.test.tsx`, because
the title, the close button and the closed-state guard are its behaviour rather
than the help sheet's:
...
Expected: PASS, 4 tests, plus 3 for `XbrwSheet`.
...
```bash
git add components/HelpSheet.tsx components/XbrwSheet.tsx \
    components/__tests__/HelpSheet.test.tsx components/__tests__/XbrwSheet.test.tsx
git commit -F - <<'EOF'
...
`DotMatrixText` already takes a `testID` and passes it to its host `Text`, so no
change is needed there. Its colour lands in `style` rather than in a `color`
prop, which is why the fourth test reads the style — and why it matches loosely:
that component composes its style as an array so that the caller's own style
cannot reach the two properties it exists to control.
...
```bash
git add components/RecipeOverflowSheet.tsx \
    components/__tests__/RecipeOverflowSheet.test.tsx
git commit -F - <<'EOF'
...
The existing `describe("useRecipeEditor volume error")` block — all four of its
tests — encodes the rule this task replaces: that `saveRecipe` refuses an
unbalanced recipe and raises a `volumeError` string. **Delete that whole block**
and the `mismatchedJSON` helper with it. What it was protecting is now covered
continuously by `balance`, which cannot go stale, rather than by a message that
had to be raised and retired by hand. Say so in the commit message.

`Recipe` has no `toJSON` method; `JSON.stringify` is what the existing tests
use.
...
The actual field write lives in a module-scope helper, because the React
Compiler's `react-hooks/immutability` rule rejects mutating a value derived
directly from state (`recipe?.pours[index]`) inside the hook body. Putting the
mutation behind a function boundary — exactly as the existing `pourFields`
writers do — satisfies it:
...
Import `type {StageField}` from `@/components/StageTile` and `Pour` from
`@/library/Pour`.
...
`volumeError`/`setVolumeError` **stay**. `useCardWriter(onVolumeError)` still
reports the write-time mismatch into this setter, and `useCardWriter.test.ts`
asserts that `autoAdjustPourVolumes()` clears it — both are out of this task's
scope. So the state and its two returned entries remain, `autoAdjustPourVolumes`
keeps its `setVolumeError(null)`, and only the imperative `editInputComplete`
clearing goes. Grep before deleting:
...
the screen now). `volumeError`/`setVolumeError` are still returned, for
`useCardWriter`.

- [ ] **Step 3b: Keep the old screen compiling**

`app/editRecipe.tsx` still consumes `totalVolumeRef`, `autoButtonRef`,
`volumeError`, `restoreOptions` and `showRestoreDialog`, and Task 14 is what
deletes that screen. Removing them from the hook without touching it would leave
`tsc` red for a whole task, so this task makes the smallest edit that keeps it
honest — no restyling, no restructuring, and no shim left inside the hook:

- Destructure `balance` and `revertSources` instead of the removed values;
  `setVolumeError` stays (it is still handed to `useCardWriter`), but the
  screen no longer reads `volumeError`.
- Hold the restore dialog's open state in the screen: `const [showRestoreDialog,
  setShowRestoreDialog] = useState(false)`, and build its `RestoreOption[]` from
  `revertSources.filter((s) => s.available)`. `restoreRecipe()` becomes a local
  function that opens it.
- Drop `ref={totalVolumeRef}` from `TotalVolumeComponent`. It re-renders from
  the `key` bump like everything else; the imperative handle was the bug.
- Drop `ref={autoButtonRef}` from the Auto button and set
  `disabled={balance.balanced}` — the same truth, read rather than pushed.
- Replace the `volumeError !== null` banner with `!balance.balanced`, keeping
  the same sentence as a literal. It is derived now, so it appears while the
  recipe does not add up rather than only after a refused save.
...
- Modify: `constants/recipeHelp.ts` (add a `name` topic)
- Test: `app/__tests__/editRecipe.test.tsx`

The recipe's own name is the one BREW row Task 3 has no help entry for, because
until now it was not a labelled field. Add one beside `xid`, matching the shape
of its neighbours — a `title` and a `hint`, no `detail`:
...
The hint is trimmed one clause shorter than first drafted: the existing
`recipeHelp.test.ts` caps hints at 64 characters and the original phrasing was
65, so it has to give a word back.

`constants/__tests__/recipeHelp.test.ts` derives its per-topic checks from
`Object.keys(RECIPE_HELP)`, so it covers the new topic without being edited, and
`DETAILED_TOPICS` excludes it automatically for having no `detail`.
...
// cannot open under jest. The editor only reads `helpStyle`; pin it.
 ...
 *
 * Both renders are wrapped in a fragment. A bare first `<EditRecipe/>` followed
 * by a fragment second render would make React see the screen's element type
 * change from `EditRecipe` to `Fragment` and remount it, cutting the caret's
 * `onPress` closure off from the live instance — so "More" would open nothing.
 * The same shape both times keeps the instance, and its overflow state, alive.
 */
...
<YStack flex={1} backgroundColor={palette.base}>
    {/* Task 15 adds the collapsing hero's `onScroll={header.onScroll}` here. */}
    {/* The key belongs on the deck, not on the scroll container. The model is
        mutated in place, so `recipe` keeps its identity across an edit and the
        deck has to be told the value moved — but keying the ScrollView remounts
        it and sends the user back to the top on every nudge. */}
    <ScrollView onScroll={onScroll} scrollEventThrottle={16}
        ...
        {deck === "brew"
            ? <BrewDeck .../>
            : <StagesPlaceholder stageCount={recipe.pours.length}/>}  {/* Task 15 replaces the placeholder */}
    </ScrollView>
    <ActionBar .../>
</YStack>
```

The real `DeckSwitch` prop is `deck`, not `value`; the sketch above is
corrected to match. `header.onScroll` and the `StagesDeck` proper both arrive in
Task 15 — this task ships a minimal `StagesPlaceholder` in that branch. The BREW
deck republishes edits by bumping the hook's `key` (the `ScrollView` carries it),
because several of the hook's field updaters do not bump it themselves.
...
|---|---|---|---|
| DOSE | `Stepper` | 1–31 g (1–10 for tea), step 1 | `RECIPE_LABELS.DOSE` |
| RATIO | `Stepper` | 5–100, step **1** | `RECIPE_LABELS.RATIO` |
| GRIND SIZE | `Stepper` | 40–80, step 1 | `RECIPE_LABELS.GRIND_SIZE` |
| GRIND SPEED | `Stepper` | 60–120, step 10 | `RECIPE_LABELS.GRIND_RPM` |
...
Those ranges were read off the current `app/editRecipe.tsx` — they are the ones
the machine accepts and they are not guessable, so do not round them off.
**Ratio steps by whole numbers only**; the card has no room for a half. Each
`Stepper`'s `label` is the matching `RECIPE_HELP` title, because that is what it
speaks: `label="Ratio"` gives "Increase Ratio" and "Ratio, 17".
...
<XStack alignItems="baseline" gap="$2">
    ...
    {/* `dim`, not `muted`: muted is 4.12:1 and the palette says in as many
        words that it is not a text colour. */}
...
`resolveAccent` and `accentGroupFor` come from `@/library/accent`; there is no
`accentFor`. `useCardWriter(setVolumeError)` returns `{writeCard,
onNFCDialogClose, showNfcOverlay, writeProgress}` and `writeCard` takes the
recipe.
...
<XStack position="absolute" bottom={0} left={0} right={0} gap="$2"
    ...
    <Pressable accessibilityRole="button" accessibilityLabel="Write card"
               ...
               onPress={() => canWrite && writeCard(recipe)} style={{flex: 2}}>
...
(`HelpTopic | "all" | null`). `openStage` (`number | null`) belongs to the
STAGES deck and is introduced with it in Task 15 — adding it here as unused
state trips `@typescript-eslint/no-unused-vars`, so it waits. Everything else
comes from `useRecipeEditor` and `useCardWriter`.

`helpStyle` is read with `useSetting("helpStyle")` and narrowed through
`asHelpStyle`; `explaining` is simply `helpStyle === "explain"`. The BREW deck's
tap-to-type rows (RECIPE ID, NAME) are an uncontrolled `Input` inside a
`FieldRow`, committing on `onEndEditing` — a controlled `value` would fight the
cursor against the in-place mutation. The STAGES branch renders a minimal
`StagesPlaceholder` that Task 15 replaces.

The old file imported `TotalVolumeComponent`, `ValidatedInput`, `LabeledInput`,
`MyButtonGroup` and `TooltipComponent`; the rebuild simply stops importing them.
Task 18 deletes the files once nothing references them.
...
```bash
git add app/editRecipe.tsx app/__tests__/editRecipe.test.tsx constants/recipeHelp.ts \
        docs/superpowers/plans/2026-08-23-editor.md
git commit -F - <<'EOF'
...
**First, give the fixture a third stage.** The tests below are written around
three, and Task 14's fixture builds two. Add one more `addPour(0)` before the
`autoFixPourVolumes()` call: 288 ml over three stages is 96 each, so Task 14's
own assertions (target 288, then 306 after the ratio nudge) are untouched.
...
function StagesDeck({recipe, balance, accent, isTea, openStage, setOpenStage,
                     editStage, addPour, deletePour, autoAdjustPourVolumes}: StagesDeckProps) {
    ...
    // The disabled tea tile dims by swapping its colours, never by opacity.
        ...
        <YStack marginTop="$3">
            ...
            {!balance.balanced && (
                <XStack testID="stage-mismatch" alignItems="center" gap="$2.5"
                    ...
                    <YStack flex={1} gap={2}>
                        {/* fontSize 11, the Doto floor: a smaller number is
                            clamped up silently, so it is written honestly. */}
                        <DotMatrixText fontSize={11} weight="bold" letterSpacing={1.6}
                                       color={palette.danger}>
            ...
            {recipe.pours.map((pour, index) => (
                <StageTile key={index} pour={pour} index={index}
                           ...
                           onChange={editStage}
                           onDelete={(i) => {
                               // Close first, or `openStage` points past the end
...
- **`deletePour` must close the open stage** or `openStage` points past the end
  of the list. The sketch above wraps it; do not pass `deletePour` in raw.
- **`addPour` takes the index it copies from, not the position to insert at.**
  `Recipe.addPour(n)` reads `pours[n]` for its values and splices in after it, so
  appending is `addPour(recipe.pours.length - 1)`. One past the end reads
  `undefined` and throws on `.volume`.
- `StageTile` keeps its body mounted and hidden through `Collapsible`, and RNTL
  excludes hidden elements by default — so "Decrease Stage volume" resolves to
  the one open tile and does not collide with the other two.
...
Expected: PASS, 14 tests across both describes.
...
And in `app/__tests__/editRecipe.test.tsx`. **`renderEditor` currently takes no
arguments and the router mock returns one fixed recipe**, so give it one first:
the `jest.mock("expo-router")` factory reads a module-level `let` — which must
keep its `mock` prefix, because Babel rejects any other name inside a hoisted
factory — and `renderEditor` sets it before rendering. Reset it in a
`beforeEach` so a tea test cannot leak into the next one.
...
**Deviation (probed):** `DotMatrixText`'s `DOTO_MIN_FONT_SIZE` is 11, so `fontSize={10}` in the sketch would have silently clamped up anyway; shipped as `fontSize={11}` per the task's own note, verified by reading `components/DotMatrixText.tsx`.

**Deviation (probed):** the insertion point in `app/editRecipe.tsx` was a placeholder comment (`{/* The tea banner is Task 16. */}`) already left by an earlier task, confirmed by reading the file; replaced it with `{recipe.isTea() && <TeaBanner accent={accent}/>}`, reusing the `accent` already computed for the hero rather than the sketch's bare `accent` prop with no visible source.
...
it: the home card carries a marker.

**The spec's second half of this — "attempting to write one from the home screen
is refused the same way" — has nothing to refuse.** There is no write action on
the home screen: `RecipeCard`'s props are `recipe, onPress, editing, onDuplicate,
onDelete, showCoffeeMarker, dottedProfile`, and `app/index.tsx` reads cards but
never writes one. Writing is reached by opening a recipe, where Task 14 already
gates it on `canWrite`. Adding a home write action to have something to refuse
would be a new feature, not this task. Dropped deliberately, not overlooked.
...
`Pour`'s volume is set directly rather than through a setter here because that
is what the surrounding tests in this file already do.
...
- [x] **Step 3: Add the marker and the refusal**
...
The marker is the whole of this task. The refusal it was to pair with already
exists, one level in: Task 14's action bar disables WRITE whenever
`balance.balanced` is false, so an unwritable recipe cannot reach the machine
from the only place it could be written. Refusing before the session matters
because an NFC write the machine rejects leaves a card in an unknown state, and
a card that will not write is not trivially recoverable.
...
should not be discoverable only by opening it and failing at the machine.

Co-authored-by: Copilot <223556219+Copilot@users.noreply.github.com>
EOF
```

**No code deviation.** The marker landed exactly as sketched, beside the existing `TEA`/`COFFEE` `DotMatrixText` inside the same `XStack`, verified by reading `components/RecipeCard.tsx` before editing it and running both new tests plus the file's other 39 to confirm nothing else moved.
...
- Delete: `components/IconButton.tsx` — not on the original list, but the old
  screen's header caret was its only caller and it is now orphaned too. The same
  argument applies: an unused component reads as the house style.
...
git rm components/IconButton.tsx
git rm components/__tests__/ValidatedInput.test.tsx \
       components/__tests__/LabeledInput.test.tsx \
       components/__tests__/MyButtonGroup.test.tsx \
       components/__tests__/TooltipComponent.test.tsx \
       components/__tests__/RestoreDialog.test.tsx
```

All but `TotalVolumeComponent` and `IconButton` had a test file.
...
Expected: all green — 731 tests across 49 suites, down from 758 across 54. The
27 tests that go with the components are the point of the task, not a loss.
`tsc` is the real check here: a missed import is a compile error, not a test
failure.
...
Two changes approved after the final review of this sub-project.

### Follow-up A: the EXPLAIN header toggle

The design (`docs/superpowers/specs/2026-08-23-editor-design.md`, Help) called
for an EXPLAIN toggle in the header: off, the screen is hint lines; on, every
note unfolds. The first cut dropped the control and hard-wired
`explaining = helpStyle === "explain"`, so with the default setting the notes
were permanently unfolded and could not be collapsed.

Restored as screen-level state — `useState(helpStyle === "explain")` — seeded
from the setting but changed for the visit only, never written back. A Doto
`EXPLAIN` caption sits beside the More caret in an `XStack` (caret last, still
the rightmost affordance), in the recipe's accent when on and `palette.dim`
when off, with `accessibilityRole`/`accessibilityLabel`/`accessibilityState`
and a `hitSlop`. It is rendered only when `helpStyle === "explain"` — the
`markers` style has nothing under the row to fold. The header effect now
depends on `explaining`, `accent` and `helpStyle` so the caption repaints.
`FieldRow`'s comment is back to "the explain toggle".

### Follow-up B: validate the recipe ID as it is typed

`inputError` was initialised `false` and, once every numeric field became a
clamping `Stepper` and every choice a fixed segmented row, was never set true —
so the `!inputError` term in `canSave`/`canWrite` was dead. The one free-text
field that can still hold something invalid, the recipe ID, was not validated
on entry at all.

`TextFieldRow` now takes `validate`, `invalidReason` and `onInvalidChange`. The
recipe-ID row passes `isValidXID` (from `library/Recipe.ts`, unchanged),
validates on every keystroke rather than on blur, reports validity up into
`inputError` (so both gates honour it), and — while invalid — colours the input
`palette.danger` and shows a short reason under the row via a new `error` prop
on `FieldRow`. The reason stays in Inter, being prose. The field is still
uncontrolled; a mount/remount effect re-reports validity so a revert to a good
ID reopens the gate the field cannot otherwise see cleared.

Dead code removed alongside it, each checked first:

- **Deleted** `enableSave` / `setEnableSave` / `initiallySaveEnabled`
  (and the `saveEnabled` route param that fed it): the flag was written
  everywhere but read nowhere, since `canSave` is `!inputError && recipe`. Its
  one load-bearing side effect — re-rendering after an `editInputComplete`
  edit — is now an explicit `setKey` bump, which is the documented publish
  mechanism for an in-place mutation.
- **Deleted** `isLoadingTitle` / `setIsLoadingTitle`: returned but unread; the
  overflow sheet's refresh-name action takes no loading prop.
- **Kept** `getRecipe`: used internally by `handleReloadTitlePress` and by
  `hooks/__tests__/useCardWriter.test.ts`.
- **Kept** `volumeError` / `setVolumeError`: `useCardWriter` reports a
  write-time mismatch into `setVolumeError` and `useCardWriter.test.ts` asserts
  `autoAdjustPourVolumes()` clears it.
