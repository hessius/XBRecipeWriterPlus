...
```ts
    name: {
        title: "Name",
        hint:  "Yours. The xBloom name is kept separately and is not overwritten."
    },
```
...
jest.mock("expo-router", () => ({
    useLocalSearchParams: () => ({recipeJSON: fixtureJSON, saveEnabled: "false"}),
...
async function renderEditor() {
    const view = await renderWithProviders(<EditRecipe/>);
...
<YStack flex={1} backgroundColor={palette.base}>
    <ScrollView onScroll={header.onScroll} scrollEventThrottle={16}
        ...
        <RecipeHero name={recipe.displayName()} named={recipe.hasName()}
                    xid={recipe.xid} accent={resolveAccent(recipe)}
